...
public class PlayerInventorySystem : MonoBehaviour
{
    ...
    public void EquipmentSlotAfter(ContainerSlot _slot)
    {
        ...
        {
            _slot.GetItem.AttributeValues[i].AddStat(character.attributeStat);
        }
        ...
        for(int i = 0; i < _slot.GetItem.GetRune.Length; i++)
        {
            ...
            {
                _slot.GetItem.GetRune[i].AttributeValues[j].AddStat(character.attributeStat);
            }
        }
        ...
        for(int i = 0; i < _slot.GetItem.ItemEffects.Length; i++)
        {
            ...
            _slot.GetItem.ItemEffects[i].Installation(character.attributeStat);
        }
    }
    ...
    public void EquipmentSlotBefore(ContainerSlot _slot)
    {
        ...
        for (int i = 0; i < _slot.GetItem.AttributeValues.Length; i++)
        {
            _slot.GetItem.AttributeValues[i].TakeStat(character.attributeStat);
        }
        ...
        for (int i = 0; i < _slot.GetItem.GetRune.Length; i++)
        {
            for (int j = 0; j < _slot.GetItem.GetRune[i].AttributeValues.Length; j++)
            {
                _slot.GetItem.GetRune[i].AttributeValues[j].TakeStat(character.attributeStat);
            }
        }
        //아이템에 붙은 효과 때는중
        for (int i = 0; i < _slot.GetItem.ItemEffects.Length; i++)
        {
            _slot.GetItem.ItemEffects[i].Uninstallation(character.attributeStat);
        }
    }
